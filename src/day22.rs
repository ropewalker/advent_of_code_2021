...
impl From<&str> for Cuboid {
    fn from(cuboid: &str) -> Self {
        ...
        Cuboid::new(
            ...
            y_range.next().unwrap().parse().unwrap(),
            z_range.next().unwrap().parse().unwrap(),
            z_range.next().unwrap().parse().unwrap(),
        )
    }
}
...
impl Cuboid {
    fn new(x_min: i32, x_max: i32, y_min: i32, y_max: i32, z_min: i32, z_max: i32) -> Self {
        Self {
            x: RangeInclusive::new(x_min, x_max),
            y: RangeInclusive::new(y_min, y_max),
            z: RangeInclusive::new(z_min, z_max),
        }
    }
...
const INIT_PROCEDURE_REGION: Cuboid = Cuboid {
    x: -50..=50,
    y: -50..=50,
    z: -50..=50,
};
...
impl From<&str> for RebootStep {
    fn from(reboot_step: &str) -> Self {
        let mut split = reboot_step.split_whitespace();

        RebootStep {
            state: split.next().unwrap().into(),
            cuboid: split.next().unwrap().into(),
        }
    }
}
...
fn non_overlapping_active_cuboids(reboot_steps: &[RebootStep]) -> Vec<Cuboid> {
    let mut active_cuboids: Vec<Cuboid> = Vec::new();

    for reboot_step in reboot_steps {
        let new_cuboid = reboot_step.cuboid.to_owned();

        match reboot_step.state {
            On => {
                ...
                for active_cuboid in active_cuboids.iter() {
                    ...
                    for cut in cuts {
                        new_cuts.extend(cut.subtract(active_cuboid));
                    }
            ...
            Off => {
                ...
                for active_cuboid in active_cuboids.into_iter() {
                    cuts.extend(active_cuboid.subtract(&new_cuboid));
                }
...
#[aoc(day22, part1)]
fn part1(reboot_steps: &[RebootStep]) -> usize {
    non_overlapping_active_cuboids(reboot_steps)
        .into_iter()
        .map(|cuboid| {
            Cuboid::new(
                cuboid.x_min().max(INIT_PROCEDURE_REGION.x_min()),
                cuboid.x_max().min(INIT_PROCEDURE_REGION.x_max()),
                cuboid.y_min().max(INIT_PROCEDURE_REGION.y_min()),
                cuboid.y_max().min(INIT_PROCEDURE_REGION.y_max()),
                cuboid.z_min().max(INIT_PROCEDURE_REGION.z_min()),
                cuboid.z_max().min(INIT_PROCEDURE_REGION.z_max()),
            )
        })
...
#[aoc(day22, part2)]
fn part2(reboot_steps: &[RebootStep]) -> usize {
    non_overlapping_active_cuboids(reboot_steps)
        .into_iter()
        .map(|cuboid| cuboid.volume())
        .sum()
...
off x=-93533..-4276,y=-16170..68771,z=-104985..-24507";
    ...
    #[test]
    fn part1_example() {
        assert_eq!(part1(&parse_input(TEST_INPUT)), 39);
        assert_eq!(part1(&parse_input(LARGER_TEST_INPUT)), 590_784);
        assert_eq!(part1(&parse_input(VERY_LARGE_TEST_INPUT)), 474_140);
    }

    #[test]
    fn part2_example() {
        assert_eq!(part2(&parse_input(TEST_INPUT)), 39);
        assert_eq!(
            part2(&parse_input(VERY_LARGE_TEST_INPUT)),
